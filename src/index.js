...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && path === "/terms") {
      return htmlResp(termsPage(), 200, { "Cache-Control": "public,max-age=3600" });
    }
    if (request.method === "GET" && path === "/privacy") {
      return htmlResp(privacyPage(), 200, { "Cache-Control": "public,max-age=3600" });
    }
    ...
    if (request.method === "POST" && path === "/share/send") {
      ...
      try {
        ...
        if (bodyStr.length > 5 * 1024 * 1024) return json({ error: "Share metadata too large" }, 400);
    ...
    if (request.method === "PUT" && path.startsWith("/share/media/")) {
      ...
      if (!shareId || shareId.length > 20 || !idx || idx.length > 10) return json({ error: "Invalid params" }, 400);
      // Verify the share exists
      const shareRaw = await env.SHARES.get(shareId);
      if (!shareRaw) return json({ error: "Share not found" }, 404);
      const body = await request.arrayBuffer();
      if (body.byteLength > 10 * 1024 * 1024) return json({ error: "File too large (10MB max)" }, 413);
      await env.MEDIA.put(`share/${shareId}/${idx}`, body, {
    ...
    if (request.method === "POST" && path === "/stripe/webhook") {
      ...
      if (event.type === "checkout.session.completed") {
        ...
        // Resolve tier from product ID in line items (no untrusted metadata fallback)
        let tier = "basic"; // default if product not recognised
        try {
    ...
    if (request.method === "GET" && path === "/stripe/success") {
      if (!await checkRateLimit(env, clientIp, "stripe_success", 30, 60)) return new Response("Too many requests", { status: 429 });
...
<div class="card">
  ...
  `}
    ...
    if (request.method === "GET" && path === "/paypal/success") {
      if (!await checkRateLimit(env, clientIp, "paypal_success", 30, 60)) return new Response("Too many requests", { status: 429 });
...
<div class="card">
  ...
  `}
    ...
    if (request.method === "GET" && path === "/paypal/key") {
      if (!await checkRateLimit(env, clientIp, "paypal_key", 20, 60)) return json({ error: "Too many requests" }, 429);
      const subscriptionId = url.searchParams.get("subscription_id") || "";
